...
bool Vector::contains(int id, int x , int y,int largo, int direccion){
    ...
    if (this->vector[id - 1]->posX == x && this->vector[id - 1]->posY == y && this->vector[id - 1]->Largo == largo && this->vector[id - 1]->Direccion == direccion)
    {
        return true;
    }
    return false;
...
void Vector::remplazar(Auto* autoNuevo){
    ...
    this->vector[autoNuevo->id - 1] = autoNuevo;
